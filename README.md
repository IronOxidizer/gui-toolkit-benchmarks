...
The time between the application start to the first complete paint **and** input handling is available. This is sent to the benchmarker by printing the epoch time in the most precise method to `stderr` when startup is complete. Startup time has the following variants:
